...
- **Skills path**: `/mnt/skills` → `medrix-flow/skills/` directory
...
| `DELETE /api/threads/{id}` | Delete MedrixFlow-managed local thread data after LangGraph thread deletion; unexpected failures are logged server-side and return a generic 500 detail |
...
For Feishu card updates, MedrixFlow stores the running card's `message_id` per inbound message and patches that same card until the run finishes, preserving the existing `OK` / `DONE` reaction flow.
...
- If a provider module is missing, MedrixFlow now returns an actionable error with install guidance (for example `uv add langchain-google-genai`).
