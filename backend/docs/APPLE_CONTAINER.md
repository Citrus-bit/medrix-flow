...
MedrixFlow now supports Apple Container as the preferred container runtime on macOS, with automatic fallback to Docker.

## Overview

Starting with this version, MedrixFlow automatically detects and uses Apple Container on macOS when available, falling back to Docker when:
...
The implementation is in `backend/packages/harness/medrix_flow/community/aio_sandbox/aio_sandbox_provider.py`:
...
```
INFO:medrix_flow.community.aio_sandbox.aio_sandbox_provider:Detected Apple Container: container version 0.1.0
INFO:medrix_flow.community.aio_sandbox.aio_sandbox_provider:Starting sandbox container using container: ...
```

Or for Docker:
```
INFO:medrix_flow.community.aio_sandbox.aio_sandbox_provider:Apple Container not available, falling back to Docker
INFO:medrix_flow.community.aio_sandbox.aio_sandbox_provider:Starting sandbox container using docker: ...
```
...
**Usage:**
```bash
# Clean up all MedrixFlow sandbox containers
./scripts/cleanup-containers.sh medrix-flow-sandbox
