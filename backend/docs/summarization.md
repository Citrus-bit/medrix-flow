...
MedrixFlow includes automatic conversation summarization to handle long conversations that approach model token limits. When enabled, the system automatically condenses older messages while preserving recent context.
...
- **Configuration**: `packages/harness/medrix_flow/config/summarization_config.py`
- **Integration**: `packages/harness/medrix_flow/agents/lead_agent/agent.py`
