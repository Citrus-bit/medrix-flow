...
def _build_runtime_middlewares(
    *,
    ...
    if include_uploads:
        from medrix_flow.agents.middlewares.uploads_middleware import UploadsMiddleware
    ...
    from medrix_flow.config.guardrails_config import get_guardrails_config
        ...
        if "framework" not in provider_kwargs:
            try:
                ...
                if "framework" in sig.parameters or any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()):
                    provider_kwargs["framework"] = "medrix_flow"
