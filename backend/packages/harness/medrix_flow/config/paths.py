...
class Paths:
    """
    Centralized path configuration for MedrixFlow application data.
    ...
    BaseDir resolution (in priority order):
        ...
        2. MEDRIX_FLOW_HOME environment variable
        3. Local dev fallback: cwd/.medrix-flow  (when cwd is the backend/ dir)
        4. Default: $HOME/.medrix-flow
    """
    ...
    def host_base_dir(self) -> Path:
        ...
        Set MEDRIX_FLOW_HOST_BASE_DIR to the host-side path that corresponds to this
        ...
        """
        if env := os.getenv("MEDRIX_FLOW_HOST_BASE_DIR"):
            return Path(env)
    ...
    def base_dir(self) -> Path:
        ...
        if env_home := os.getenv("MEDRIX_FLOW_HOME"):
            return Path(env_home).resolve()

        cwd = Path.cwd()
        if cwd.name == "backend" or (cwd / "pyproject.toml").exists():
            return cwd / ".medrix-flow"

        return Path.home() / ".medrix-flow"
    ...
    def experiment_db_file(self) -> Path:
        """Path to the experiment persistence database."""
