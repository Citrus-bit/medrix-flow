...
from medrix_flow.config.app_config import get_app_config, reset_app_config
...
def _write_config(path: Path, *, model_name: str, supports_thinking: bool) -> None:
    path.write_text(
        yaml.safe_dump(
            {
                "sandbox": {"use": "medrix_flow.sandbox.local:LocalSandboxProvider"},
                "models": [
                    {
                        "name": model_name,
...
def test_get_app_config_reloads_when_file_changes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MEDRIX_FLOW_CONFIG_PATH", str(config_path))
...
def test_get_app_config_reloads_when_config_path_changes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MEDRIX_FLOW_EXTENSIONS_CONFIG_PATH", str(extensions_path))
    monkeypatch.setenv("MEDRIX_FLOW_CONFIG_PATH", str(config_a))
    reset_app_config()
    ...
    try:
        ...
        monkeypatch.setenv("MEDRIX_FLOW_CONFIG_PATH", str(config_b))
