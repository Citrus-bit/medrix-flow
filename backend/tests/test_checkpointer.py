...
import medrix_flow.config.app_config as app_config_module
from medrix_flow.agents.checkpointer import get_checkpointer, reset_checkpointer
from medrix_flow.config.checkpointer_config import (
    CheckpointerConfig,
...
class TestGetCheckpointer:
    def test_returns_in_memory_saver_when_not_configured(self):
        ...
        with patch("medrix_flow.agents.checkpointer.provider.get_app_config", side_effect=FileNotFoundError):
            cp = get_checkpointer()
...
# ---------------------------------------------------------------------------
# MedrixFlowClient falls back to config checkpointer
# ---------------------------------------------------------------------------


class TestClientCheckpointerFallback:
    def test_client_uses_config_checkpointer_when_none_provided(self):
        """MedrixFlowClient._ensure_agent falls back to get_checkpointer() when checkpointer=None."""
        from langgraph.checkpoint.memory import InMemorySaver
        ...
        with (
            patch("medrix_flow.client.get_app_config", return_value=config_mock),
            patch("medrix_flow.client.create_agent", side_effect=fake_create_agent),
            patch("medrix_flow.client.create_chat_model", return_value=MagicMock()),
            patch("medrix_flow.client._build_middlewares", return_value=[]),
            patch("medrix_flow.client.apply_prompt_template", return_value=""),
            patch("medrix_flow.client.MedrixFlowClient._get_tools", return_value=[]),
        ):
            client = MedrixFlowClient(checkpointer=None)
    ...
    def test_client_explicit_checkpointer_takes_precedence(self):
        ...
        with (
            ...
            client = MedrixFlowClient(checkpointer=explicit_cp)
            config = client._get_runnable_config("test-thread")
            client._ensure_agent(config)
