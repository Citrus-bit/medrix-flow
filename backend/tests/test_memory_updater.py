...
from medrix_flow.agents.memory.prompt import format_conversation_for_update
from medrix_flow.agents.memory.updater import MemoryUpdater, _extract_text
...
def test_apply_updates_skips_existing_duplicate_and_preserves_removals() -> None:
    ...
    with patch(
        "medrix_flow.agents.memory.updater.get_memory_config",
...
def test_apply_updates_skips_same_batch_duplicates_and_keeps_source_metadata() -> None:
    ...
    update_data = {
        "newFacts": [
            ...
            {"content": "User works on MedrixFlow", "category": "context", "confidence": 0.87},
        ],
    }
...
class TestUpdateMemoryStructuredResponse:
    ...
    def test_string_response_parses(self):
        ...
        with (
            ...
            patch("medrix_flow.agents.memory.updater.get_memory_data", return_value=_make_memory()),
            patch("medrix_flow.agents.memory.updater._save_memory_to_file", return_value=True),
        ):
            msg = MagicMock()
            msg.type = "human"
            msg.content = "Hello"
