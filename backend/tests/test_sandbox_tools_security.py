...
from medrix_flow.sandbox.tools import (
    VIRTUAL_PATH_PREFIX,
    _is_skills_path,
...
_THREAD_DATA = {
    "workspace_path": "/tmp/medrix-flow/threads/t1/user-data/workspace",
    "uploads_path": "/tmp/medrix-flow/threads/t1/user-data/uploads",
    "outputs_path": "/tmp/medrix-flow/threads/t1/user-data/outputs",
}
...
def test_replace_virtual_path_maps_virtual_root_and_subpaths() -> None:
    assert (
        ...
        == "/tmp/medrix-flow/threads/t1/user-data/workspace/a.txt"
    )
    assert Path(replace_virtual_path("/mnt/user-data", _THREAD_DATA)).as_posix() == "/tmp/medrix-flow/threads/t1/user-data"
...
def test_mask_local_paths_in_output_hides_host_paths() -> None:
    output = "Created: /tmp/medrix-flow/threads/t1/user-data/workspace/result.txt"
    masked = mask_local_paths_in_output(output, _THREAD_DATA)

    assert "/tmp/medrix-flow/threads/t1/user-data" not in masked
...
def test_mask_local_paths_in_output_hides_skills_host_paths() -> None:
    ...
    with (
        patch("medrix_flow.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"),
        patch("medrix_flow.sandbox.tools._get_skills_host_path", return_value="/home/user/medrix-flow/skills"),
    ):
        output = "Reading: /home/user/medrix-flow/skills/public/bootstrap/SKILL.md"
        masked = mask_local_paths_in_output(output, _THREAD_DATA)

        assert "/home/user/medrix-flow/skills" not in masked
...
def test_validate_local_tool_path_rejects_traversal_in_skills() -> None:
    ...
    with patch("medrix_flow.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"):
        with pytest.raises(PermissionError, match="path traversal"):
...
def test_validate_local_tool_path_rejects_none_thread_data() -> None:
    ...
    from medrix_flow.sandbox.exceptions import SandboxRuntimeError
...
def test_resolve_skills_path_resolves_correctly() -> None:
    ...
    with (
        ...
        assert resolved == "/home/user/medrix-flow/skills/public/bootstrap/SKILL.md"
...
def test_resolve_skills_path_resolves_root() -> None:
    ...
    with (
        ...
        assert resolved == "/home/user/medrix-flow/skills"
...
def test_resolve_skills_path_raises_when_not_configured() -> None:
    ...
    with (
        patch("medrix_flow.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"),
        patch("medrix_flow.sandbox.tools._get_skills_host_path", return_value=None),
    ):
...
def test_replace_virtual_paths_in_command_replaces_skills_paths() -> None:
    ...
    with (
        ...
        assert "/home/user/medrix-flow/skills/public/bootstrap/SKILL.md" in result
...
def test_replace_virtual_paths_in_command_replaces_both() -> None:
    ...
    with (
        patch("medrix_flow.sandbox.tools._get_skills_container_path", return_value="/mnt/skills"),
        patch("medrix_flow.sandbox.tools._get_skills_host_path", return_value="/home/user/skills"),
    ):
        ...
        assert "/tmp/medrix-flow/threads/t1/user-data/workspace/out.txt" in result
...
def test_validate_local_tool_path_skills_custom_container_path() -> None:
    ...
    with patch("medrix_flow.sandbox.tools._get_skills_container_path", return_value="/custom/skills"):
        # Should not raise
        validate_local_tool_path(
