...
medrix_flow.subagents.executor. This test file uses delayed import via fixture to test
...
_MOCKED_MODULE_NAMES = [
    ...
    "medrix_flow.agents.middlewares",
    "medrix_flow.agents.middlewares.thread_data_middleware",
    "medrix_flow.sandbox",
    "medrix_flow.sandbox.middleware",
    "medrix_flow.models",
]
...
def _setup_executor_classes():
    ...
    original_executor = sys.modules.get("medrix_flow.subagents.executor")

    # Remove mocked executor if exists (from conftest.py)
    if "medrix_flow.subagents.executor" in sys.modules:
        del sys.modules["medrix_flow.subagents.executor"]
    ...
    from medrix_flow.subagents.executor import (
        SubagentExecutor,
        SubagentResult,
        SubagentStatus,
    )
    ...
    if original_executor is not None:
        sys.modules["medrix_flow.subagents.executor"] = original_executor
    elif "medrix_flow.subagents.executor" in sys.modules:
        del sys.modules["medrix_flow.subagents.executor"]
...
class TestCleanupBackgroundTask:
    ...
    def executor_module(self, _setup_executor_classes):
        ...
        from medrix_flow.subagents import executor
