...
class TestDeferredToolsPromptSection:
    ...
    def _mock_app_config(self, monkeypatch):
        ...
        from medrix_flow.config.tool_search_config import ToolSearchConfig

        mock_config = MagicMock()
        mock_config.tool_search = ToolSearchConfig()  # disabled by default
        monkeypatch.setattr("medrix_flow.config.get_app_config", lambda: mock_config)

    def test_empty_when_disabled(self):
        from medrix_flow.agents.lead_agent.prompt import get_deferred_tools_prompt_section
...
class TestDeferredToolFilterMiddleware:
    ...
    def _ensure_middlewares_package(self):
        ...
        That file replaces medrix_flow.agents and medrix_flow.agents.middlewares with
        ...
        mock_keys = [
            ...
            "medrix_flow.agents.middlewares.deferred_tool_filter_middleware",
        ]
