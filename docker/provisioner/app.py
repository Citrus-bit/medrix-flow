"""MedrixFlow Sandbox Provisioner Service.
...
K8S_NAMESPACE = os.environ.get("K8S_NAMESPACE", "medrix-flow")
SANDBOX_IMAGE = os.environ.get(
    "SANDBOX_IMAGE",
    "enterprise-public-cn-beijing.cr.volces.com/vefaas-public/all-in-one-sandbox:latest",
)
SKILLS_HOST_PATH = os.environ.get("SKILLS_HOST_PATH", "/skills")
THREADS_HOST_PATH = os.environ.get("THREADS_HOST_PATH", "/.medrix-flow/threads")
        ...
        if exc.status == 404:
            ns = k8s_client.V1Namespace(
                metadata=k8s_client.V1ObjectMeta(
                    name=K8S_NAMESPACE,
                    labels={
                        "app.kubernetes.io/name": "medrix-flow",
...
app = FastAPI(title="MedrixFlow Sandbox Provisioner", lifespan=lifespan)
...
async def list_sandboxes():
    ...
    try:
        services = core_v1.list_namespaced_service(
            K8S_NAMESPACE,
            label_selector="app=medrix-flow-sandbox",
        )
    except ApiException as exc:
        raise HTTPException(
