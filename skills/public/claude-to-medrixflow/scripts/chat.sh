#!/usr/bin/env bash
# chat.sh — Send a message to MedrixFlow and collect the streaming response.
#
# Usage:
...
#   MEDRIXFLOW_URL=http://host:2026 bash chat.sh "hi"   # custom endpoint
#
# Environment variables:
#   MEDRIXFLOW_URL          — Unified proxy base URL (default: http://localhost:2026)
#   MEDRIXFLOW_GATEWAY_URL  — Gateway API base URL (default: $MEDRIXFLOW_URL)
#   MEDRIXFLOW_LANGGRAPH_URL — LangGraph API base URL (default: $MEDRIXFLOW_URL/api/langgraph)
#
...
GATEWAY_URL="${MEDRIXFLOW_GATEWAY_URL:-$MEDRIXFLOW_URL}"
LANGGRAPH_URL="${MEDRIXFLOW_LANGGRAPH_URL:-$MEDRIXFLOW_URL/api/langgraph}"
...
if [ "$HTTP_CODE" = "000" ] || [ "$HTTP_CODE" -ge 400 ]; then
  echo "ERROR: MedrixFlow is not reachable at ${GATEWAY_URL} (HTTP ${HTTP_CODE})" >&2
  echo "Make sure MedrixFlow is running. Start it with: cd <medrix_flow-dir> && make dev" >&2
  exit 1
fi
...
if result_messages is not None:
    ...
    for event_type, data_str in events:
        if event_type == "error":
            print(f"ERROR from MedrixFlow: {data_str}", file=sys.stderr)
            sys.exit(1)
